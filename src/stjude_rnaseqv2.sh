...
main() {
    ...
    miniwdl run -d $HOME/output https://raw.githubusercontent.com/stjudecloud/workflows/rnaseq-standard/v3.0.0/workflows/rnaseq/rnaseq-standard.wdl gencode_gtf=gencode.gtf.gz input_bam=$input_bam_name stardb_tar_gz=STARDB.tar.gz strandedness=$strand output_prefix=$output_prefix detect_nproc=true validate_input=false bam_to_fastqs.split.reject_unaccounted=false > output.json
    ...
    # Copy any properties from input bam to the counts file and output bam
    id=$(echo $input_bam | jq -r '.["$dnanexus_link"]')
    json=$(dx describe --json $id | jq -r ".properties")
    dx api $gene_counts setProperties "{\"properties\": $json, \"project\": \"${DX_WORKSPACE_ID}\"}"
    dx api $bam setProperties "{\"properties\": $json, \"project\": \"${DX_WORKSPACE_ID}\"}"
